...
var compressDir = function(dir, outfile, doUpdate, callback) {
...
var createPackage = function(dir, outfile, callback) {
  ...
  if (!dirExists) {
    err = new Error('Path doesn\'t exist');
    if (callback) {
      callback(err);
    }
    return log.error(err);
  ...
  if (!stats.isDirectory()) {
    err = new Error('%s is not a directory');
    if (callback) {
      callback(err);
    }
    return log.error(err);
  ...
  if (!outfileExists) {
    compressDir(dir, outfile, true, callback);
    return;
  }
